...
pub fn plot(
    ...
    while date <= end_day {
        let daily_work_time =
            single_day_work_time(log_location.clone(), date).expect("Failed to get work time");
...
fn plot_helper(
    ...
    let root = BitMapBackend::new(&plot_location, (1280, 780)).into_drawing_area();
    root.fill(&WHITE)?;
    ...
    let line_series = LineSeries::new(dots.iter().map(|(dt, v)| (*dt, *v)), &BLUE.mix(0.7));
    chart.draw_series(line_series)?;

    chart.draw_series(
        dots.iter()
            .map(|(dt, v)| Circle::new((*dt, *v), 4, BLUE.mix(0.7).filled())),
    )?;
